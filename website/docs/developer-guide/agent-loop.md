...
- **Preflight, inside the agent loop** (50% threshold): `ContextCompressor.threshold_percent` defaults to `0.50` in `agent/context_compressor.py`, which derives an absolute `threshold_tokens` value (`int(context_length * threshold_percent)`) that is the actual runtime trigger. Before each API call, `run_conversation()` estimates request size with `estimate_request_tokens_rough()` over messages, system prompt, and tool schemas (preflight block inside `run_conversation()` in `agent/conversation_loop.py`, ~line 436) and fires compression when the estimate reaches `threshold_tokens` (50% of the model's context window at the default `threshold_percent`). After the provider replies, Hermes prefers the real API-reported `last_prompt_tokens` value (post-response compression block inside `run_conversation()`, ~line 3382) and only falls back to `estimate_request_tokens_rough()` when usage data is missing — that is the path that can fire an additional post-call compression pass.
