...
The summary model **must** have a context window at least as large as your main agent model's. The compressor sends the full middle section of the conversation to the summary model — if that model's context window is smaller, the summarization call will fail with a context length error. When this happens, Hermes inserts a deterministic extractive fallback summary with redacted snippets from the middle window and surfaces fallback metadata so the gateway or `/compress` path can warn you that compaction quality degraded. If you override the model, verify its context length meets or exceeds your main model's.
:::
