"""Timestamp and auto-continue-freshness helpers extracted from ``gateway/run.py``.

Part of the hermes-monolith-split refactor (Phase 1). Pure functions; the
only module-level state is the freshness-window default
(``_AUTO_CONTINUE_FRESHNESS_SECS_DEFAULT``).

Why auto-continue freshness exists: stale tool-tail/resume markers can
otherwise revive an unrelated old task after a gateway restart when the
user's next message starts new work. The freshness signal is the timestamp
of the last transcript row, which ``hermes_state.get_messages`` carries on
every persisted message. This handles the two auto-continue cases uniformly:

* resume_pending (gateway restart/shutdown watchdog marked the session)
* tool-tail     (last persisted message is a tool result the agent never
                  got to reply to)
...
gets interrupted near its timeout boundary and is resumed shortly after is
still classified fresh. Override via ``config.yaml``
