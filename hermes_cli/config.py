...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Hermes was installed: 'nixos', 'homebrew', 'docker', 'git', or 'pip'."""
    ...
    try:
        stamped = stamp.read_text(encoding="utf-8").strip().lower()
        if stamped:
            return stamped
    except OSError:
        pass
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "docker":
        return "docker pull ghcr.io/nousresearch/hermes-agent:latest && docker compose up -d"
...
def load_config_readonly() -> Dict[str, Any]:
    """Backward-compatible read-only config loader.

    Historically this avoided migrations/writes; load_config() is now cached
    and side-effect-free for normal reads, so keep the public symbol as a thin
    alias for callers such as timeout resolution.
    """
    return load_config()


def stamp_install_method(method: str) -> None:
    """Persist a best-effort install-method marker for postinstall flows."""
    marker = get_hermes_home() / ".install_method"
    try:
        marker.write_text(str(method or "").strip() + "\n", encoding="utf-8")
    except Exception as exc:
        logger.debug("Failed to stamp install method %r: %s", method, exc)
    ...
    *,
    ...
    if "discover_models" in entry:
        ...
        if isinstance(discover, str):
            normalized["discover_models"] = discover.lower() not in {"false", "no", "0"}
        else:
            normalized["discover_models"] = bool(discover)
